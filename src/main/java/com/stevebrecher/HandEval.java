 ...
 * to seven cards represented within a long (64 bits). The long is considered as
 ...
 * <p>
 * For high evaluation if results R1 > R2, hand 1 beats hand 2; for lowball
 * evaluation if results R1 > R2, hand 2 beats hand 1.
 * <p>
 * Evaluation result in 32 bits = 0x0V0RRRRR where V, R are hex digits or
 * "nybbles" (half-bytes).
 * <p>
 * V nybble = category code ranging from
 * {@link HandCategory#NO_PAIR}<code>.ordinal()</code> to
 * {@link HandCategory#STRAIGHT_FLUSH}<code>.ordinal()</code>
 * <p>
 * The R nybbles are the significant ranks (0..12), where 0 is the deuce in a
 * high result (Ace is 12, 0xC), and for lowball 0 is the Ace (King is 0xC). The
 * Rs may be considered to consist of Ps for ranks which determine the primary
 * value of the hand, and Ks for kickers where applicable. Ordering is
 * left-to-right: first the Ps, then any Ks, then padding with 0s. Because 0 is
 * a valid rank, to interpret a result you must know how many ranks are
 * significant, which is a function of the hand category and whether high or
 * lowball. Examples: for a one-pair hand there are four significant ranks, that
 * of the pair and of the three kickers; for a straight, there is one
 * significant rank, that of the highest in the hand.
 * <p>
 ...
 *  Razz, wheel:  0x00043210</pre> For the eight-or-better lowball ..._Eval
 * functions, the result is either as above or the constant {@link #NO_8_LOW}.
 * NO_8_LOW > any other ..._Eval function result.
 * <p>
 * @version 2010Jun25.1
...
public final class HandEval {
     ...
     * evaluation methods. The value may also be bitwise OR'ed or added to other
     * such values to build an evaluation method parameter.
     */
    /*
    ...
    private static int flushAndOrStraight7(final int ranks, final int c, final int d, final int h, final int s) {
        ...
        } else /* total cards in other suits <= 7-5: spade flush: */ if ((i = straightValue[s]) == 0) {
 ...
 /* bits for singleton, if any, and trips, if any: */
                ...
                if ((j = c & d & h & s) != 0) /* bit for quads */ /* quads with pair and singleton */ {
		 ...
		 * or quads with singleton kicker and two non-playing singletons
                ...
                if (nbrOfRanks[i] == 1) {
                    /* three pair and singleton */
                    j = hiUpTo5Ranks[ranks ^ i];
                    /* ranks of the three pairs */
		 ...
		 * or trips and four singletons
                ...
                if (nbrOfRanks[i] != 5) /* two pair and three singletons */ {
		 ...
		 * or one pair and three kickers and two nonplaying singletons
                ...
                i = c ^ d ^ h ^ s;
                /* the bits of the five singletons */
 ...
 /* bits for singleton, if any, and trips, if any: */
                ...
                if ((j = c & d & h & s) != 0) /* bit for quads */ /* AAAABBC -- two pair */ {
    ...
    private static int flushAndOrStraight6(final int ranks, final int c, final int d, final int h, final int s) {
        ...
        } else /* total cards in other suits <= N-5: spade flush: */ if ((i = straightValue[s]) == 0) {
            return FLUSH | hiUpTo5Ranks[s];
        } else {
            return (STRAIGHT_FLUSH - STRAIGHT) + i;
        }
 ...
 /* bits for trips, if any: */
                if ((nbrOfRanks[i = c ^ d ^ h ^ s]) != 0) /* two trips (full house) */ {
                    return FULL_HOUSE | (hiUpTo5Ranks[i] << RANK_SHIFT_3);
                }
                /* quads with pair kicker */
                i = c & d & h & s;
            ...
            case 3:
                ...
                if ((c ^ d ^ h ^ s) == 0) /* no trips or singletons:  three pair */ {
                ...
                if ((i = c & d & h & s) == 0) {
                    ...
                    j = c ^ d ^ h ^ s;
                    /* the bits of the trips and singleton */
            ...
            case 4:
                ...
                if ((i = c ^ d ^ h ^ s) != ranks) /* two pair and two singletons */ {
                    return TWO_PAIR | (hiUpTo5Ranks[i ^ ranks] << RANK_SHIFT_3) | (hiRank[i] << RANK_SHIFT_2);
                }
                /* trips and three singletons */
                if ((i = c & d) == 0) {
            ...
            case 5:
                ...
                i = c ^ d ^ h ^ s;
                /* the bits of the four singletons */
    ...
    public static int hand5Eval(long hand) {
        ...
        switch (nbrOfRanks[ranks]) {
            ...
            case 2:
                ...
                } else /* the quads bit must be present in each suit mask,
            ...
            case 5:
                ...
                } /*      return straight or no pair value */ else if (d != 0) {
                    if (d != ranks) {
                        return i;
                    }
                } else if (h != 0) {
 ...
 /* There is a flush */
    ...
    /**
     * Returns the Deuce-to-Seven low (Kansas City lowball) value of a 5-card
     * poker hand.
     *
     * @param hand bit mask with one bit set for each of 5 cards.
     * @return the value of the hand.
    ...
    /**
     * Returns the bitwise OR of the suit masks comprising <code>hand</code>;
     * Ace is high.
     *
     ...
     * @return the 8-or-better low value of <code>hand</code> or
     * {@link #NO_8_LOW}.
     */
    ...
    /**
     * Returns the 8-or-better low value of a 5-card poker hand comprised of
     * three board cards and two hole cards or {@link #NO_8_LOW}.
     *
    ...
    private static void setStraight(int ts) {
        ...
        for (i = 0x1000; i > 0; i >>= 1) {
            for (j = 0x1000; j > 0; j >>= 1) {
                es = ts | i | j;
                /* 5 straight bits plus up to two other bits */
